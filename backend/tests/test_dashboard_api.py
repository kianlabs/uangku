...
def client(test_engine):
    ...
    with TestClient(app, raise_server_exceptions=True) as c:
        c.post("/api/v1/auth/register", json={"email": "dash@test.com", "password": "pass"})
        c.post("/api/v1/auth/login", json={"email": "dash@test.com", "password": "pass"})
        yield c
...
def other_client(test_engine):
    ...
    app.dependency_overrides[deps.get_db] = override_get_db
    with TestClient(app, raise_server_exceptions=True) as c:
        c.post("/api/v1/auth/register", json={"email": "dash_other@test.com", "password": "pass"})
        c.post("/api/v1/auth/login", json={"email": "dash_other@test.com", "password": "pass"})
        yield c
    app.dependency_overrides.clear()
