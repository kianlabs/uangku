...
def test_metrics_today_expense_only_counts_today(
    db, metrics_user, metrics_income_cat, metrics_expense_cat
):
    today = date(2029, 4, 15)
    # Counted: expense exactly today
    create_transaction(db, metrics_user, type_="expense", amount=Decimal(30000),
                       category_id=metrics_expense_cat.id, transaction_date=today)
    # Not counted: expense yesterday
    create_transaction(db, metrics_user, type_="expense", amount=Decimal(999999),
                       category_id=metrics_expense_cat.id, transaction_date=date(2029, 4, 14))
    # Not counted: income today
    create_transaction(db, metrics_user, type_="income", amount=Decimal(888888),
                       category_id=metrics_income_cat.id, transaction_date=today)
